...
function readSingleFile_F(evt) {
  var f = evt.target.files[0];
  if (f) {
    var r = new FileReader();
    r.onload = function (e) {
      var contents = e.target.result;
      var data = processCSVData(contents, 3);
      _content_F = data[1];
      console.log("F " + _content_F.length);
      console.log(_content_F[0]);
      if (error1 == 0)
        document.getElementById("button_next_" + currentPage).style.display = "";
    }
    r.readAsText(f);
...
document.getElementById('fileinput_f').addEventListener('change', readSingleFile_F, false);
  ...
  if (error1 == 0) {
    for (var i = 0; i < _content.length; ++i) {
      ...
      for (var j = 0; j < header2.length; ++j) {
        ...
        if (_content[i][j] != "" && _content[i][j] != " " && (lastLine == null || _content[i][j] != lastLine[j])) {
          // if (extraDB_Index == 4)
          // console.log("|" + _content[i][j] + "|" + _content[i][j].length + "|" + (_content[i][j] == " \r"));
          hasData = true;
        }
      }
      if (hasData)
        final_content.push(obj);
      // _content[i] = obj;
      lastLine = _content[i];
    }
  }

  return [_header, final_content];
}
...
function upload_F() {
  numUploaded = 0;
  document.getElementById("upload_progress_F").innerHTML = "Deleting original data";
  deleteFromDB('parts_db/F', function () {
    console.log("Deletion complete! ");
    for (var i = 0; i < _content_F.length; ++i) {
      var newRef = getNewKeyPath("parts_db/F");
      writeToDB(newRef, _content_F[i], function () {
        ++numUploaded;
        document.getElementById("upload_progress_F").innerHTML = "Uploaded " + numUploaded + " of " + _content_F.length;
        if (numUploaded == _content_F.length)
          upload_GEM();
      });
    }
  });
