  ...
  if (content_div_visible) {
    if (!pressedKeys[KEY_CTRL_LEFT] && !pressedKeys[KEY_SHIFT_LEFT] && !pressedKeys[KEY_ALT_LEFT]
      ...
      if (event.code == KEY_ESCAPE) {
        ...
        else if (!_focused && clickInvoiceHistory_ExitInvoice()) { //Don't check for TAB_INVOICE_HISTORY because the invoice view can be reached from TAB_PART_HISTORY
        ...
        for (var i = 0; i <= 9; ++i) {
          if (event.code == "Digit" + i) {
            ...
            if (!_focused && _selected_tab == TAB_SAVED_SEARCHES) {
              var ele = document.getElementById("loadSavedSearch_" + i);
              if (ele != null)
                ele.click();
            }
            break;
        ...
        switch (event.code) {
          ...
          case KEY_DOWN_ARROW: //Down Arrow
            ...
            break;
          case KEY_PAGE_UP:
            if (_isTableSelected && (_selected_tab == TAB_RECORD_BROWSER || _selected_tab == TAB_SEARCH_RESULTS)) {
              if (pageUp())
                event.preventDefault();
            }
            else if (!_focused && _selected_tab == TAB_RECORD_VIEWS) {
          ...
          case KEY_C:
            if (shortcutmenu_mainmenu_available)
              ...
              else if (document.getElementById("googlesearch_image_div").style.display != "none") {
                document.getElementById("radio_image_self").click();
              }
            }
            else if (!_focused && _selected_tab == TAB_RECORD_BROWSER) {
          ...
          case KEY_S:
            ...
            else if (searchInCurrentResults(event)) {
    ...
    if (!pressedKeys[KEY_CTRL_LEFT] && !pressedKeys[KEY_SHIFT_LEFT] && !pressedKeys[KEY_ALT_LEFT]
      ...
      if (event.code == KEY_ESCAPE) {
        if (_focused) {
          ...
          if (ele1.style.display != "none") {
            printBackArrowPressed();
          }
        }
      }
      else if (event.code == KEY_PAGE_UP) {
        exportPageUp(event);
      }
      else if (event.code == KEY_PAGE_DOWN) {
        exportPageDown(event);
      }
    }
  }
}