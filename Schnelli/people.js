...
function insertEmployeeIDTableRow(first_name, middle_name, last_name, id, admin) {
    ...
    if (admin)
        tableHTML += "checked";
    tableHTML += "><span title='Admins are allowed to make changes in the people tab' style='border-bottom: 1px dotted black;'> ? </span></td><td><button style='width: 100px; height: 30px;' onclick='deleteEmployee(" + (len - 1) + ");'>X</button></td>";
...
function insertClientTableRow(email, first_name, last_name, company, blacklisted, uid, writeable) {
    ...
    // if (blacklisted)
    //     tableHTML += "checked";
    // tableHTML += "><span title='Blacklisted Clients are banned from accessing the database' style='border-bottom: 1px dotted black;'> ? </span></td><td style='display: none;'>" + uid + "</td>";
    // tableHTML += "<td><input style='width: 30px; height: 30px;' onchange='clientTableChanged();' type='checkbox'";
    // if (writeable)
    //     tableHTML += "checked";
    // tableHTML += "><span title='Writeable Client is allowed to make changes to the database and have employees' style='border-bottom: 1px dotted black;'> ? </span></td><td><button class='largeText' style='width: 150px;' onclick='viewInfo(" + (len - 1) + ");'>Info</button></td>";
    // row.innerHTML = tableHTML;
}
