...
function setKeyboardShortcutBar()
{
  ...
  if(_LOGGED_IN)
  {
    ...
    if(!_focused && _selected_tab == TAB_SEARCH_RESULTS)
    {
      ...
      text += "&nbsp;&nbsp;<span style='color: white;'>S</span>earch in Current Results";
    }
...
input_email.addEventListener("keyup", function(event) {
  ...
  function moveUp()
  {
    ...
    if(_selectedTable == _TABLE_RECORD_BROWSER)
    {
      if(_selectedRow == 0)
      {
        nextRBIndex = getNextContentIndexInSortOrder(_indexesRecordBrowser[0], _contentSortedReverse);
        if(nextRBIndex == null)
          preventDefault = false;
      }
    }
  ...
  function moveDown(){
    ...
    if(_selectedTable == _TABLE_RECORD_BROWSER)
    {
      ...
      {
        nextRBIndex = getNextContentIndexInSortOrder(_indexesRecordBrowser[_recordBrowserMax - 1], !_contentSortedReverse);
        if(nextRBIndex == null)
          preventDefault = false;
      }
    }
