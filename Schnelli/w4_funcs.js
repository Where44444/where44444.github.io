...
function onCellClick(row, cell, id, table_enum, skipPopulate) {
  ...
  if (table_enum != _TABLE_RECORD_BROWSER || (textele_edit == null && textele_new == null)) //Prevents record browser from being selected while editing
  {
    ...
    if (elementCell != null) {
      ...
      elementCell.scrollIntoView({ behavior: "auto", block: "nearest", inline: "nearest" }); //Need to do this scroll for horizontal viewing
      eleSmartScroll(elementCell, 103, 60); //This scroll for vertical viewing
...
function saveContentToDatabase(rownum, addChangeAlert) {
  ...
  if (!_DEBUG_LOCAL_MODE) {
    ...
    writeToDatabase('parts_db/P&A_PRI/' + row[row.length - 1], partObj, addChangeAlert, true, false, null, rownum);
  }
}
...
function writeToDatabase(key_path, value, addChangeAlert, is_content, is_content_extra, content_extra_db, content_rownum_for_MODIFIED) {
  var paths = key_path.split("/");
  if (content_rownum_for_MODIFIED != null) {
    edit_content(content_rownum_for_MODIFIED, "MODIFIED", getMMDDYYYYText(new Date(), "/"));
    if (is_content)
      value["MODIFIED"] = getMMDDYYYYText(new Date(), "/");
    else
      if (!_DEBUG_LOCAL_MODE)
        saveContentToDatabase(content_rownum_for_MODIFIED, true);
  }
  ...
  if (is_content_extra && value != null) { //Calculate retail price
    ...
    var fixed_n = Number(fixed);
    cgs = standardizeString(cgs);
    reg = standardizeString(reg);
    fixed = standardizeString(fixed);
    if (isNaN(cgs_n))
      cgs_n = 0;
    if (isNaN(reg_n))
      reg_n = 0;
    if (isNaN(fixed_n))
      fixed_n = 0;
    if (fixed_n != 0) {
      value.CALCULATED = get_USD_String(fixed_n);
    }
    else if (cgs == "" && reg == "")
      value.CALCULATED = "0.00";
    else if (reg == "" || reg_n == 0)
      value.CALCULATED = get_USD_String(cgs_n * 2);
    else if (reg_n < 0.5)
      value.CALCULATED = get_USD_String(reg_n + 0.5);
    else if (reg_n < 1)
      value.CALCULATED = get_USD_String(reg_n + 1);
    else if (reg_n < 5)
      value.CALCULATED = get_USD_String(reg_n * 1.95);
    else if (reg_n < 15)
      value.CALCULATED = get_USD_String(reg_n * 1.85);
    else if (reg_n < 25)
      value.CALCULATED = get_USD_String(reg_n * 1.65);
    else if (reg_n < 50)
      value.CALCULATED = get_USD_String(reg_n * 1.5);
    else
      value.CALCULATED = get_USD_String(reg_n * 1.35);
    if (paths != null && paths.length > 0) {
      var key = paths[paths.length - 1];
      var rownum = getContentExtraIndexFrom_DB_ID(key, content_extra_db);
      if (rownum != null) {
        _content_extra[content_extra_db][rownum][0][CE_CALCULATED] = value.CALCULATED;
      }
    }

    // if (paths != null && paths.length > 0) {
    //   var parentRownum = getParentRecordIndexWithChildPart_IncludingAKA(content_extra_db, getContentExtraIndexFrom_DB_ID(paths[paths.length - 1], content_extra_db));
    //   if (parentRownum != null) {
    //     edit_content(parentRownum, "MODIFIED", getMMDDYYYYText(new Date(), "/"));

    //     if (!_DEBUG_LOCAL_MODE)
    //       saveContentToDatabase(parentRownum, true);
    //   }
    // }
  }
...
function saveContentExtraToDatabase(i, j) {
  if (!_DEBUG_LOCAL_MODE) {
    ...
    var parentRownum = getParentRecordIndexWithChildPart_IncludingAKA(i, j)
    writeToDatabase("parts_db/" + _EXTRA_DB[i] + "/" + _content_extra[i][j][1], extraobj0, true, false, true, i, parentRownum);
  }
  _CHILD_PART_LINKS_CACHE.clear();
}
...
const TAB_MAINMENU_DIVS = [
  ...
  'Save<span style="color:white">d</span> Searches',
  'Truc<span style="color:white">k</span>s'
];

const SUBSCRIBED_DIVS = [
  true, //TAB_SEARCH
  true, //TAB_RECORD_VIEWS
  true, //TAB_RECORD_BROWSER
  true, //TAB_PART_CHILD_RECORD_MANAGER
  true, //TAB_SORT_ORDERS
  false, //TAB_PART_HISTORY
  false, //TAB_PDF_IMPORT
  false, //TAB_REORDERS
  false, //TAB_INVOICE_HISTORY
  false, //TAB_INVOICE_SETTINGS
  false, //TAB_INVOICE
  false, //TAB_MAINMENU
  true, //TAB_SEARCH_RESULTS
  false, //TAB_ADD_INVOICE 
  false, //TAB_PEOPLE
  true, //TAB_SUGGESTIONS
  false, //TAB_CHANGE_HISTORY
  false, //TAB_EXPORT
  true, //TAB_SAVED_SEARCHES
  false //TAB_TRUCKS
];
...
var _invoice_history_filters = ["", "", "", "", ""];
var _part_history_filters = ["", "", "", "", ""];
var _change_history_filters = ["", "", "", "", ""];
function setTab(num, inhibitPushBrowserState) {
  var html = "";

  if (!isMobileDevice()) { //DESKTOP
    _top_bar_height = document.getElementById("nav_bar").offsetHeight + "px";
    if (num == TAB_PART_HISTORY) {
      html =
        ...
        + '</tr>'
        + '<tr>'
        + '  <td><input onchange="savePartHistoryFilters();" style="width: 200px;" type="text" id="part_history_filter_time" name="part_history_filter_time" value="' + _part_history_filters[0] + '"></td>'
        + '  <td><input onchange="savePartHistoryFilters();" type="text" id="part_history_filter_pn" value="' + _part_history_filters[1] + '" oninput="populatePartHistory();"></td>'
        + '  <td><input onchange="savePartHistoryFilters();" type="text" id="part_history_filter_desc" value="' + _part_history_filters[2] + '" oninput="populatePartHistory();"></td>'
        + '  <td><input onchange="savePartHistoryFilters();" type="text" id="part_history_filter_price" value="' + _part_history_filters[3] + '" oninput="populatePartHistory();"></td>'
        + '  <td><input onchange="savePartHistoryFilters();" type="text" id="part_history_filter_part_any_field" value="' + _part_history_filters[4] + '" oninput="populatePartHistory();">'
        + '  </td>'
        + '</tr>';
    }
    if (num == TAB_INVOICE_HISTORY) {
      html =
        ...
        + '</tr>'
        + '<tr>'
        + '  <td><input onchange="saveInvoiceHistoryFilters();" style="width: 200px;" type="text" id="invoice_history_filter_time"'
        + '      name="invoice_history_filter_time" value="' + _invoice_history_filters[0] + '">'
        + '  </td>'
        + '  <td><input onchange="saveInvoiceHistoryFilters();" type="text" id="invoice_history_filter_name" value="' + _invoice_history_filters[1] + '" oninput="populateInvoiceHistory();">'
        + '  </td>'
        + '  <td><input onchange="saveInvoiceHistoryFilters();" type="text" id="invoice_history_filter_total" value="' + _invoice_history_filters[2] + '" oninput="populateInvoiceHistory();">'
        + '  </td>'
        + '  <td><input onchange="saveInvoiceHistoryFilters();" type="text" id="invoice_history_filter_invoice_no" value="' + _invoice_history_filters[3] + '"'
        + '      oninput="populateInvoiceHistory();"></td>'
        + '  <td><input onchange="saveInvoiceHistoryFilters();" type="text" id="invoice_history_filter_invoice_any_field" value="' + _invoice_history_filters[4] + '"'
        + '      oninput="populateInvoiceHistory();"></td>'
        + '</tr>';
    }
    if (num == TAB_CHANGE_HISTORY) {
      html =
        ...
        + '</tr>'
        + '<tr>'
        + '  <td><input onchange="saveChangeHistoryFilters();" style="width: 200px;" type="text" id="change_history_filter_time" name="change_history_filter_time" value="' + _change_history_filters[0] + '"></td>'
        + '  <td><input onchange="saveChangeHistoryFilters();" type="text" id="change_history_filter_initials" value="' + _change_history_filters[1] + '" oninput="populateChangeHistory();"></td>'
        + '  <td><input onchange="saveChangeHistoryFilters();" type="text" id="change_history_filter_type" value="' + _change_history_filters[2] + '" oninput="populateChangeHistory();"></td>'
        + '  <td><input onchange="saveChangeHistoryFilters();" type="text" id="change_history_filter_change" value="' + _change_history_filters[3] + '" oninput="populateChangeHistory();"></td>'
        + '  <td><input onchange="saveChangeHistoryFilters();" type="text" id="change_history_filter_change_any_field" value="' + _change_history_filters[4] + '" oninput="populateChangeHistory();"></td>'
        + '</tr>';
    }
  }
  else { //MOBILE
    if (num == TAB_PART_HISTORY) {
      html =
        ...
        + '<tr>'
        + '  <td>Any Field</td>'
        + '  <td><input onchange="savePartHistoryFilters();" type="text" id="part_history_filter_part_any_field" value="' + _part_history_filters[4] + '" oninput="populatePartHistory();"></td>'
        + '</tr>'
    }
    if (num == TAB_INVOICE_HISTORY) {
      html =
        '<tr>'
        + '  <td>Date</td>'
        + '  <td><input onchange="saveInvoiceHistoryFilters();" style="width: 200px;" type="text" id="invoice_history_filter_time" name="invoice_history_filter_time" value="' + _invoice_history_filters[0] + '"></td>'
        + '</tr>'
        + '<tr>'
        + '  <td>Name</td>'
        + '  <td><input onchange="saveInvoiceHistoryFilters();" type="text" id="invoice_history_filter_name" value="' + _invoice_history_filters[1] + '" oninput="populateInvoiceHistory();"></td>'
        + '</tr>'
        + '<tr>'
        + '  <td>Total</td>'
        + '  <td><input onchange="saveInvoiceHistoryFilters();" type="text" id="invoice_history_filter_total" value="' + _invoice_history_filters[2] + '" oninput="populateInvoiceHistory();"></td>'
        + '</tr>'
        + '<tr>'
        + '  <td>Invoice No.</td>'
        + '  <td><input onchange="saveInvoiceHistoryFilters();" type="text" id="invoice_history_filter_invoice_no" value="' + _invoice_history_filters[3] + '" oninput="populateInvoiceHistory();"></td>'
        + '</tr>'
        + '<tr>'
        + '  <td>Any Field</td>'
        + '  <td><input onchange="saveInvoiceHistoryFilters();" type="text" id="invoice_history_filter_invoice_any_field" value="' + _invoice_history_filters[4] + '" oninput="populateInvoiceHistory();"></td>'
        + '</tr>';
    }
    if (num == TAB_CHANGE_HISTORY) {
      html =
  ...
  //Mobile style re-arrangment
  if (num == TAB_PART_HISTORY) {
    document.getElementById("part_history_filter_table").innerHTML = html;

    $(function () {
      $('input[name=part_history_filter_time]').daterangepicker({
        showDropdowns: true,
        startDate: _part_filter_date_start,
        endDate: _part_filter_date_end,
        locale: {
          format: 'MM/DD/YYYY'
        }
  ...
  if (num == TAB_INVOICE_HISTORY) {
    document.getElementById("invoice_history_filter_table").innerHTML = html;
    ...
    $(function () {
      $('input[name=invoice_history_filter_time]').daterangepicker({
        ...
        showDropdowns: true,
        startDate: _invoice_filter_date_start,
        endDate: _invoice_filter_date_end,
  ...
  if (num == TAB_CHANGE_HISTORY) {
    document.getElementById("change_history_filter_table").innerHTML = html;
    $(function () {
      $('input[name=change_history_filter_time]').daterangepicker({
        // timePicker: true,
        // singleDatePicker: true,
        showDropdowns: true,
        startDate: _change_filter_date_start,
        endDate: _change_filter_date_end,
        // startDate: moment().startOf('hour'),
        locale: {
          format: 'MM/DD/YYYY'
        }
  ...
  if (num == TAB_PEOPLE) {
    if (_current_employee == null || _current_employee.admin == null || !_current_employee.admin)
      return;
    else
  ...
  if (_selected_tab == TAB_INVOICE) //Leaving Invoice Tab MAKE SURE THIS HAPPENS BEFORE clearInvoicesContent() BELOW!
  {
    saveInvoiceToObject();
  }
  ...
  document.getElementById("search_scope_indicator").style.display = "none";
  deselectTable();
  ...
  if (num == TAB_SAVED_SEARCHES) {
    updateSavedSearches();
  }

  setKeyboardShortcutBar();

  if (!inhibitPushBrowserState)
...
function get_Percent_String(num) {
  if (isNaN(num))
    num = 0;
  num = String(Math.round(num * 10));
  if (num.length > 1)
    return num.substring(0, num.length - 1) + "." + num.substring(num.length - 1, num.length);
  else if (num.length == 1)
    return "0." + num;
  return "0.0";
}
...
function getMaxContentIndexInSortOrder(forward, valid_positions, filterString, closestIndex) {
  ...
  if (forward) //Forward order
  {
    ...
    for (var j = 0; j < _content.length; ++j) //Pick up at closest index to optimize searching, no need to check previous indexes because they didn't match filter
    {
      if (filterString == null || doesContentMatchFilter(j, filterString)) {
...
function compareContentSortingPositions(pos1, pos2) {
  for (var k = 0; k < _contentSortedIndexes.length; ++k) {
    ...
    //Causes problems trying to go from index like 16700 to 16700~
    // var num1 = Number(str1);
    // var num2 = Number(str2);
    // if (str1.length > 0 && str1[str1.length - 1] == '"' || str1[str1.length - 1] == "'" || str1[str1.length - 1] == "~"){
    //   num1 = Number(str1.substring(0, str1.length - 1));
    // }
    // if (str2.length > 0 && str2[str2.length - 1] == '"' || str2[str2.length - 1] == "'" || str2[str2.length - 1] == "~"){
    //   num2 = Number(str2.substring(0, str2.length - 1));
    // }
...
function getContentExtraObj(extradb, index, arr) {
  if (arr != null) {
    ...
    for (var i = 0; i < _EXTRA_DB_FIELDS[extradb].length; ++i) {
      ...
      if (obj[_EXTRA_DB_FIELDS[extradb][i]] == undefined)
        obj[_EXTRA_DB_FIELDS[extradb][i]] = null;
    }
    return obj;
  }
  else {
...
function checkForFirestoreSubscription(userid, func) {
  firestore_db.collection("customers")
    ...
    .onSnapshot(async (snapshot) => {
      if (snapshot.empty) {
        showSnackbar("⚠ Error: No active subscriptions were found for this account. <a href='./stripe.html' style='color: lightblue;'>Subscribe here</a>", 10000);
      } else {
        showSnackbar("Please Wait: Your subscription is still being processed, it should be ready within 5 minutes", 5000);
      }
      func();
    });
}

var _mobileInitial = -1;
// var _mobileInitial = false;
function isMobileDevice() {
  if (_mobileInitial == -1)
    _mobileInitial = (typeof window.orientation !== "undefined") || (navigator.userAgent.indexOf('IEMobile') !== -1);
  return _mobileInitial;
};

function setNavBar(open) {
  if (open) {
    document.getElementById("nav_bar_toggle").style.display = "none";
    document.getElementById("nav_bar").style.display = "flex";
  } else {
    document.getElementById("nav_bar_toggle").style.display = "block";
