...
function generatePDFAddToDatabaseTable(index) {
  ...
  if (stock_ele == null)
    stock_ele = document.getElementById("pdf_shipped_" + index);
  var descrip = "";
  var make = "";
  var date = "";
...
function confirmPDFAddToDatabase(index) {
  ...
  if (!_DEBUG_LOCAL_MODE) {
    var parentRownum = getParentRecordIndexWithChildPart_IncludingAKA(extradb, link);
    var partObj0 = getContentExtraObj(extradb, link);
    writeToDatabase("parts_db/" + _EXTRA_DB[extradb] + "/" + _content_extra[extradb][link][1], partObj0, true, false, true, extradb, parentRownum);
...
// var _barcode_found_pn = "W10116565";
var _barcode_found_pn = "W1011656";
function barcode_Add_RecordView() {
  if (_barcode_found_pn != "") {
    var index = null;
    var db_index = null;
    for (var i = 0; i < _EXTRA_DB.length; ++i) {
      index = getExtraDBLinkIndex(i, _barcode_found_pn);
      if (index != null) {
        db_index = i;
        break;
      }
    ...
    if (index != null) {
      var parent = getParentRecordIndexWithChildPart(db_index, index);
      if (parent != null) {
        addRecordView(_content[parent][_content[parent].length - 1]);
  ...
  showSnackbar("Unable to find match in database", 3000);
}

function barcode_Add_To_Inventory() {
  if (_barcode_found_pn != "") {
    var index = null;
    var db_index = null;
    for (var i = 0; i < _EXTRA_DB.length; ++i) {
    ...
    if (index != null) {
      var num = Number(_content_extra[db_index][index][0][CE_SHOP_QTY]);
      if (isNaN(num))
        num = 0;
      _content_extra[db_index][index][0][CE_SHOP_QTY] = num + 1;
      saveContentExtraToDatabase(db_index, index);
      showSnackbar("Added 1 to stock | Current is " + _content_extra[db_index][index][0][CE_SHOP_QTY], 3000);
      return;
    }
  }
  ...
  for (var i = 0; i < INDEXES_CONCAT.length; ++i)
    if (i < _INDEXES.length)
      ...
      row.push([""]);
  row[_OEM_PN] = _barcode_found_pn;
  newParentRecord(row);
  _content[_content.length - 1][_OEM_PN] = _barcode_found_pn;

  var numFields = _EXTRA_DB_FIELDS[8].length; //OEM
  var newObj = Array.apply(null, Array(numFields)).map(function () { }) //Init arrays
  for (var i = 1; i < numFields; ++i)
    newObj[i] = "";
  newObj[CE_PN] = _barcode_found_pn;
  newObj[CE_SHOP_QTY] = "1";
  saveNewPartChildObj(8, newObj);

  addRecordView(_content[_content.length - 1][_content[_content.length - 1].length - 1]);
  _record_view_to_edit = _recordViews.length - 1;
}

function barcode_Add_To_Invoice() {
  if (_barcode_found_pn != "") {
    var index = null;
    var db_index = null;
    for (var i = 0; i < _EXTRA_DB.length; ++i) {
    ...
    if (index != null) {
      var parent_record_index = getParentRecordIndexWithChildPart_IncludingAKA(db_index, index);
      addtoinvoice(1, db_index, index, parent_record_index);
      showSnackbar("Added 1 to Invoice", 5000);
      return;
    }
  }
  showSnackbar("Unable to find match in database", 3000);
...
function startZxing() {
  ...
  document.getElementById("zxing_div").style.display = "";
  document.getElementById("scan_barcode").style.display = "none";
  document.getElementById("barcode_results").style.display = "none";
    ...
    .then((videoInputDevices) => {
      ...
      if (videoInputDevices.length > 1) {
        ...
        videoInputDevices.forEach((element) => {
          ...
          const sourceOption = document.createElement('input')
          sourceOption.type = 'radio'
          sourceOption.name = 'sourceSelect'
          sourceOption.style.width = "50px";
          sourceOption.style.height = "50px";
          if (selectedDeviceId == element.deviceId)
            sourceOption.checked = true;
          // sourceOption.innerHTML = element.label
          sourceOption.value = element.deviceId
          sourceSelect.appendChild(sourceOption)
          const label = document.createElement('span')
          label.innerHTML = element.label + "<br>"
          label.style.fontSize = '20pt';
          sourceSelect.appendChild(label)
          ++i;

          sourceOption.onchange = () => {
            selectedDeviceId = sourceOption.value;
            startZxing();
      ...
      codeReader.decodeOnceFromVideoDevice(selectedDeviceId, 'video').then((result) => {
        _barcode_found_pn = result.text;
        document.getElementById("zxing_div").style.display = "none";
        document.getElementById("scan_barcode").style.display = "";
        document.getElementById("barcode_results").style.display = "";
        document.getElementById("barcode_result_code").innerHTML = _barcode_found_pn;
        codeReader.reset();
        ...
        if (!_reset_pressed) {
          ...
          document.getElementById("scan_barcode").style.display = "";
          document.getElementById("barcode_results").style.display = "";
          document.getElementById("barcode_result_code").innerHTML = err;
        }
      })
...
function setTorch(enabled) {
  _codeReader.stream.getVideoTracks().forEach((track) => {
...
//Has trouble decoding from camera
// function startQuagga() {
//   document.getElementById("scan_barcode").style.display = "none";
//   document.getElementById("barcode_div").style.display = "";
//   document.getElementById("barcode_results").style.display = "none";
//   $(function () {
...
//       // console.log(code);
//       _barcode_found_pn = code;
//       Quagga.stop();
//       document.getElementById("barcode_div").style.display = "none";
//       document.getElementById("scan_barcode").style.display = "";
//       document.getElementById("barcode_results").style.display = "";
//       document.getElementById("barcode_result_code").innerHTML = code;
...
// function toggle_quagga_controls() {
//   if (document.getElementById("quagga_controls").style.display == "none")
//     document.getElementById("quagga_controls").style.display = "";
//   else
//     document.getElementById("quagga_controls").style.display = "none";
// }
