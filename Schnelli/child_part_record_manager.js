...
function populateChildPartRecordManager() {
  ...
  if (_selected_child_part_db != null && _selected_child_part_record != null) {
    ...
    htmlToAdd += "<div style='display: flex; flex-direction: row;'><table>";
    ...
    for (var i = 1; i < _EXTRA_DB_FIELDS[_selected_child_part_db].length; ++i) {
      ...
      if (i % _MAX_TABLE_ROWS == 0)
        htmlToAdd += "</table><table>"; //Start a new column
      htmlToAdd += "<tr><th>" + key + "</th>";
      if (i == CE_CALCULATED)
        htmlToAdd += "<td><input type='text' onfocus='deselectTable();' id='partchild_edit_input_" + key + "' value='" + getHTMLSafeText(value) + "' style='width: 250px;' title='This is a calculated field that takes REG and CGS into account' disabled></td></tr>";
      else
        htmlToAdd += "<td><input type='text' onfocus='deselectTable();' id='partchild_edit_input_" + key + "' value='" + getHTMLSafeText(value) + "' style='width: 250px;'></td></tr>";
    }
    htmlToAdd += "</table></div>";
...
function startNewPartChild() {
  ...
  if (db_index < _EXTRA_DB_COMMENTS_PREFIXES.length) {
    var htmlToAdd =
      ...
      + "<div style='display: flex; flex-direction: row;'><table>";
    for (var i = 1; i < _EXTRA_DB_FIELDS[db_index].length; ++i) {
      if (i % _MAX_TABLE_ROWS == 0)
        htmlToAdd += "</table><table>"; //Start a new column
      htmlToAdd += "<tr><th>" + _EXTRA_DB_FIELDS[db_index][i] + "</th>";
      htmlToAdd += "<td><input type='text' onfocus='deselectTable();' id='partchild_new_input_" + _EXTRA_DB_FIELDS[db_index][i] + "' style='width: 250px;'></td></tr>";
    }
    htmlToAdd += "</table></div>";
...
function saveEditPartChild() {
  ...
  if (!_DEBUG_LOCAL_MODE) {
    var parentRownum = getParentRecordIndexWithChildPart_IncludingAKA(_selected_child_part_db, _selected_child_part_record);
    var newObj0 = getContentExtraObj(_selected_child_part_db, _selected_child_part_record);
    writeToDatabase("parts_db/" + _EXTRA_DB[_selected_child_part_db] + "/" + _content_extra[_selected_child_part_db][_selected_child_part_record][1], newObj0, true, false, true, _selected_child_part_db, parentRownum);
...
function saveNewPartChild() {
  ...
  if (!_DEBUG_LOCAL_MODE) {
    ...
    writeToDatabase("parts_db/" + _EXTRA_DB[db_index] + "/" + newRef.key, newObj0, true, false, true, db_index, null); //Saving a new part child likely means there is no parent for it to update MODIFIED for
...
function saveNewPartChildObj(db_index, newObj) {
  var newRef = getDatabaseRef("parts_db/" + _EXTRA_DB[db_index]).push();
  _content_extra[db_index].push([newObj, newRef.key])
  if (!_DEBUG_LOCAL_MODE) {
    var newObj0 = getContentExtraObj(db_index, _content_extra[db_index].length - 1);
