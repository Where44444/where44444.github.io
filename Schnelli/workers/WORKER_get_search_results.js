onmessage = function (e) {
    ...
    var _SPEC_DATA_INDEXES = e.data[12];
    ...
    var specdata_ranges = [];
    // var regexp_number_range = new RegExp(/(\d*\.?\d+)( *)-( *)(\d*\.*\d+)/, "g"); //Don't make global regexp objects! Doesn't always match as intended
    ...
    for (var i = 0; i < searchstring_specific.length; ++i) {
        ...
        if (_SPEC_DATA_INDEXES.includes(i)) {
            if (columnsToSearch[i]) {
                var str = searchstring_specific[i];
                //Simplify fractions to decimal (1/4 -> 0.25)
                //Check for x.xxx - x.xxx format
                //Add to specdata_ranges if found, null if not
                var match1 = null;
                var numerator1 = null;
                ...
                var match0;
                var regexp_number_fraction = new RegExp(/(\d+)( *)\/( *)(\d+)/, "g");
                while (match2 == null && (match0 = regexp_number_fraction.exec(str)) !== null) {
                    //match is an array where 0 is the whole match (football) and each item after is the grouping (foo)(t)(ball)
                    // console.log(`Found ${match[0]} start=${match.index} end=${regexp.lastIndex}.`);
                    // expected output: "Found football start=6 end=14."
                    // expected output: "Found foosball start=16 end=24."
                    if (match1 == null) {
                        match1 = match0[0];
                        numerator1 = Number(match0[1]);
                        denominator1 = Number(match0[4]);
                    }
                    else if (match2 == null) {
                        match2 = match0[0];
                        numerator2 = Number(match0[1]);
                        denominator2 = Number(match0[4]);
                    }
                }

                if (match1 != null && denominator1 != 0) //Avoid divide by zero errors
                {
                    str = str.replace(match1, numerator1 / denominator1);
                }

                if (match2 != null && denominator2 != 0) //Avoid divide by zero errors
                {
                    str = str.replace(match2, numerator2 / denominator2);
                }

                match0 = (new RegExp(/(\d*\.?\d+)( *)-( *)(\d*\.*\d+)/, "g")).exec(str);
                if (match0 != null) {
                    specdata_ranges.push([Number(match0[1]), Number(match0[4])]);
                }
                else
                    specdata_ranges.push(null);
            } //if(columnsToSearch[i])
            else
                specdata_ranges.push(null);
        } //if(_SPEC_DATA_INDEXES.includes(i))
    ...
    for (var i = 0; i < content.length; ++i) {
        ...
        if (currentResultsIndexesIDs.length == 0 || currentResultsIndexesIDs.includes(content[i][content[i].length - 1])) {
            ...
            {
                for (var j = 0; j < row.length - 1 && continueSearchingRow; ++j) //Continue searching row gets set to false if a cell in the row doesn't match one of the AND inputs
                {

                    // 1/4 0.025 100 3/4 500
                    // 0.25 0.025 100 0.75 500
                    if (columnsToSearch[j]) {
                        if (_SPEC_DATA_INDEXES.includes(j) && specdata_ranges[j] != null) {
                            var origStr = String(row[j]);
                            var content_to_search = origStr;
                            var match0;
                            var fraction_matches = [];
                            var regexp_number_fraction = new RegExp(/(\d+)( *)\/( *)(\d+)/, "g");
                            while ((match0 = regexp_number_fraction.exec(content_to_search)) !== null) //Get all fractions in cell
                                fraction_matches.push(match0);

                            var dec_to_frac_map = new Map();
                            for (var k = 0; k < fraction_matches.length; ++k) {
                                var match0 = fraction_matches[k];
                                var decimal = Number(match0[1]) / Number(match0[4]);
                                content_to_search = content_to_search.replace(match0[0], decimal);
                                dec_to_frac_map.set(decimal, match0);
                            }

                            var matchFound0 = false;
                            var regexp_number = new RegExp(/(\d*\.?\d+)/, "g");
                            while (!matchFound0 && (match0 = regexp_number.exec(content_to_search)) !== null) //Get all numbers
                            {
                                var num = Number(match0[0]);
                                if (num >= specdata_ranges[j][0] && num <= specdata_ranges[j][1]) { //A match (of many possibly) found in cell
                                    match.row = i + _ROW_OFFSET;
                                    match.id = row[row.length - 1]; //Row ID
                                    match.columns.push(j);
                                    match.ranking += RANKING_WHOLE_EXACT;
                                    if (dec_to_frac_map.has(num)) {
                                        var match00 = (new RegExp(dec_to_frac_map.get(num)[0], "g")).exec(origStr);
                                        if (match00 != null)
                                            match.strings.push(origStr.substring(match00.index, match00.index + match00[0].length));
                                    }
                                    else {
                                        var match00 = (new RegExp(num, "g")).exec(origStr);
                                        if (match00 != null)
                                            match.strings.push(origStr.substring(match00.index, match00.index + match00[0].length));
                                    }
