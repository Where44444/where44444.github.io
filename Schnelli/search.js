function search_query(clearResults, fromSaved) {
  ...
  if (clearResults == null) {
    ...
    if (!fromSaved) {
      ...
      for (let ele0 of _SEARCH_ELES) {
        var ele = document.getElementById(ele0);
        if (ele != null) {
          if (anyChecked) {
            if (ele0 == "search_input")
              savedSearch.push(ele.value);
          }
          else {
            if (ele0 == "search_input")
              savedSearch.push("");
      ...
      if (_selected_searchQuery_scope == _SEARCHSCOPE_CURRENT) {
        if (_current_saved_search != null) {
          _saved_searches.push(copyArray2D(_saved_searches[_current_saved_search]));
          _current_saved_search = _saved_searches.length - 1;
          _saved_searches[_current_saved_search].push(savedSearch);
        }
        else {
          _current_saved_search = 0;
          _saved_searches.push([savedSearch]);
        }
      }
      else {
        _saved_searches.push([savedSearch]);
...
function searchQueryScope(scope, clearResults) {
  ...
  if (searchstring_any == "" && anyChecked) {
    ...
    for (var i = 0; i < total_indexes_length; ++i) {
      if (document.getElementById("search_input_" + i) != null) {
        ...
        columnsToSearch.push(anyChecked || document.getElementById("search_input_" + i).value != "");
          // columnsToSearch.push(anyChecked || document.getElementById("column_checkbox_" + i).checked);
      }
      else {
        searchstrings_specific.push("");
        columnsToSearch.push(false);
      }
    }
        ...
        if (results[0] == -1) {
          ...
          if (searchFinished) {
            if (_searchResults.length == 0) {
              ...
              showSnackbar("Search complete, Sorted by Relevance", 1000);
...
let page_nav_search = "<button style='width: 100px; height: 40px;' onclick='pageUp();'>&lt; Prev Page</button>&nbsp;&nbsp;&nbsp;<button style='width: 100px; height: 40px;' onclick='pageDown();'>Next Page &gt;</button>&nbsp;&nbsp;&nbsp;<button id='searchInResultsButton' onclick='searchInCurrentResults(event);' >S<span style='color: black;'>earch in Current Results</span></button>";
...
function populateSearchResults(indexStart, selectTopRow, selectBottomRow, rowToSelect, populateFromBottom) {
  if (!_adding_child_part_links && _searchResults.length > 0) {
    ...
    var _table_HTML_0 = "<p style='display: inline;'>Showing " + _indexesSearchResults.length + " of " + _searchResults.length + " Result(s)</p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" +
      ...
      for (var i = 0; i < array_trimmed.length; ++i) {
        ...
        for (var j = 0; j < INDEXES_CONCAT.length; ++j) {
          ...
          _table_HTML_0 +=
            ...
            "&nbsp;&nbsp;&nbsp;<img id='record_view_icon_search_" + i + "_" + j + "' title='Open Record View' src='record_view.png' width=50px height=20px style='display: " + display + ";' onclick='addRecordView(\"" + array_trimmed[i][array_trimmed[i].length - 1] + "\");'></div></td>"
            ;
        }
        _table_HTML_0 += "</tr>";
      }
    ...
    else //Should add child part links
    {
      ...
      for (var i = 0; i < NUM_ADD_CHILD_PART_HTML_THREADS; ++i) {
        ...
        childPartButtonWorker.postMessage([_EXTRA_DB, _EXTRA_DB_COMMENTS_PREFIXES, _content_extra, array_trimmed_slice, INDEXES_CONCAT, _DESCRIP1, _INDEX_ORDER, _DESCRIP2, _COMMENTS, _TABLE_SEARCH_RESULTS, _CHILD_PART_LINKS_CACHE, _EXTRA_DB_FIELDS, CE_AKA, _extradb_link_index_cache, orig_indexStart, i, CE_PN, isMobileDevice()]);
...
function finishPopulateSearchResults(_table_HTML_0) {
  _table_HTML_0 += "</table>" + page_nav_search;
  ...
  if (_current_saved_search2 != null)
    loadSavedSearch();
}
...
function show_more_column_checkboxes(visible) {
  if (visible) {
    ...
    document.getElementById("radio_columns_checkboxes_more").style.display = "flex";
  }
  else {
...
function searchResultsJumpToEdge(forward) {
  var closestIndex = 0;
  var i = 0;
  for (let b of _search_results_indexes_bool) {
    if (b) {
      closestIndex = i;
      break;
    }
  ...
  var index = 0;
  if (_search_results_resorted) {
    index = getMaxContentIndexInSortOrder(forward, _search_results_indexes_bool, null, closestIndex);
    var i = 0;
    for (let r of _searchResults) {
      if (index == r.row) {
        index = i;
        break;
      }
  ...
  populateSearchResults(index, !forward, forward, forward * _searchResultsMax);
}
...
function searchInCurrentResults(event) {
  if (_key_shortcut_search_results_search_in_current_results) {
    setTab(TAB_SEARCH);
    ...
    document.getElementById("search_scope_indicator").style.display = "inline";
    return true;
  }
  return false;
}