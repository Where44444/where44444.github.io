...
let _top_bar_height = "106px"; //Light green

var _settings = [];
function initialMobileSettings(override) { //Made complex to allow switching between mobile and desktop modes while debugging, call this function from console in web browser to update
  if (_settings.length == 0) {
    _settings.push(document.getElementById("nav_bar").style.width);
    _settings.push(document.getElementById("nav_bar").style.padding);
    _settings.push(document.getElementById("nav_bar").style.flexDirection);
    _settings.push(document.getElementById("desktop_nav_spacer").style.display);
    _settings.push(document.getElementById("search_partnum_db_inputs_div").style.width);
    _settings.push(document.getElementById("key_shortcut_info_bar").style.display);
    _settings.push(document.getElementById("key_shortcut_info_bar").style.opacity);
    _settings.push(document.getElementById("key_shortcut_info_bar").style.zIndex);
    _settings.push(_top_bar_height);
    _settings.push(document.getElementById("TAB_record_browser").style.display);
    _settings.push(document.getElementById("TAB_part_child_record_manager").style.display);
  }
  if (override != 1 && (isMobileDevice() || override == 0)) { //Mobile
    _mobileInitial = true;
    setNavBar(false);
    document.getElementById("nav_bar").style.width = "200px";
    document.getElementById("nav_bar").style.padding = "5px";
    document.getElementById("nav_bar").style.flexDirection = "column";
    document.getElementById("desktop_nav_spacer").style.display = "none";
    document.getElementById("search_partnum_db_inputs_div").style.width = "90%";
    document.getElementById("key_shortcut_info_bar").style.display = "none";
    document.getElementById("key_shortcut_info_bar").style.opacity = 0;
    document.getElementById("key_shortcut_info_bar").style.zIndex = -1;
    _top_bar_height = "0px";
    document.getElementById("TAB_record_browser").style.display = "none";
    document.getElementById("TAB_part_child_record_manager").style.display = "none";
  }
  else { //Desktop
    _mobileInitial = false;
    setNavBar(true);
    document.getElementById("nav_bar").style.width = _settings[0];
    document.getElementById("nav_bar").style.padding = _settings[1];
    document.getElementById("nav_bar").style.flexDirection = _settings[2];
    document.getElementById("desktop_nav_spacer").style.display = _settings[3];
    document.getElementById("search_partnum_db_inputs_div").style.width = _settings[4];
    document.getElementById("key_shortcut_info_bar").style.display = _settings[5];
    document.getElementById("key_shortcut_info_bar").style.opacity = _settings[6];
    document.getElementById("key_shortcut_info_bar").style.zIndex = _settings[7];
    _top_bar_height = _settings[8];
    document.getElementById("TAB_record_browser").style.display = _settings[9];
    document.getElementById("TAB_part_child_record_manager").style.display = _settings[10];
  }
}

initialMobileSettings(-1);
...
const RECORD_VIEW_HEADERS_PAGE1_0 = ["MFR", "PN", "T", "S"];
const RECORD_VIEW_HEADERS_WIDTHS_PAGE1_0 = ["100px", "200px", "70px"];
const RECORD_VIEW_HEADERS_PAGE1_1 = ["LOC", "U", "VEND", "DATE", "CGS", "RETAIL", "COMMENT", "SELL"];
const RECORD_VIEW_HEADERS_WIDTHS_PAGE1_1 = ["100px", "70px", "70px", "70px", "100px", "200px", "100px", "100px", "50px", "100px"];
const RECORD_VIEW_HEADERS_PAGE2_0 = ["MFR", "PN", "AKA"];
const RECORD_VIEW_HEADERS_WIDTHS_PAGE2_0 = ["100px", "300px", "70px"];
const RECORD_VIEW_HEADERS_PAGE2_1 = ["MFR2", "DATE", "CGS", "SUGG", "FIXED", "COMMENT", "SELL"]; //Needs to be MFR2 for editing to work!
const RECORD_VIEW_HEADERS_WIDTHS_PAGE2_1 = ["100px", "70px", "70px", "70px", "100px", "100px", "200px"];
...
const RECORD_VIEW_HEADERS_ACTUAL_INDEXES_PAGE1_0 = [
  ...
  null, //Filled in depending on Employee's truck contents
...
const RECORD_VIEW_HEADERS_ACTUAL_INDEXES_PAGE1_1 = [
  ...
  [CE_CALCULATED, CE_CALCULATED, CE_CALCULATED, CE_CALCULATED, CE_CALCULATED, CE_CALCULATED, CE_CALCULATED, CE_CALCULATED, CE_CALCULATED]
];
...
const RECORD_VIEW_HEADERS_ACTUAL_INDEXES_PAGE2_CONCAT = RECORD_VIEW_HEADERS_ACTUAL_INDEXES_PAGE2_0.concat(RECORD_VIEW_HEADERS_ACTUAL_INDEXES_PAGE2_1);
...
for (var i = 0; i < _EXTRA_DB.length; ++i) { //Display: none in first div so we can hide it (per David's request) without breaking other dependants 
  htmlToAdd +=
    "<div style='display: none; margin-right: 5px;'><span style='font-size: 18px;'>" + _EXTRA_DB[i] + "</span><br>" +
    "<input id='search_partnum_input_" + i3 + "' type='text' style='width: 230px;' onfocus='deselectTable();' onkeyup='search_partnum_input_keyup_event(event, " + i3 + ");' oninput='search_partnum_input_keyup_event(event, " + i3 + ");' onkeydown='search_partnum_input_keydown_event(event);'>" +
    "<div id='search_partnum_autocomplete_" + i3 + "' style=''></div></div>";
  ++i3;
}
document.getElementById("search_partnum_db_inputs_div").innerHTML = htmlToAdd;

// checkboxHTML += "<button id='search_specific_button' style='display: inline; width: 100px;' onclick='search_query();'>Go</button>";
for (var i = 0; i < INDEXES_CONCAT.length; ++i) {
  var order_id = _INDEX_ORDER[i];
  if (i <= 9) {
    checkboxHTML += "<div>";
    // checkboxHTML += "<label class='checkBox_container' style='display: inline; font-size: 18px; padding-left: 0px;'>" + INDEXES_CONCAT[order_id] + "<input type='checkbox' tabindex='-1' id=\"column_checkbox_" + order_id + "\" disabled><span class='checkmark' style='display: none;'></span></label><br>";
    checkboxHTML += "<label class='checkBox_container' style='display: inline; font-size: 18px; padding-left: 0px;'>" + INDEXES_CONCAT[order_id] + "</label><br>";
    checkboxHTML += "<input id='search_input_" + order_id + "' type='text' style='width: 230px;' onfocus='deselectTable(" + order_id + ");' onfocusout='onSearchInputFocusOut();' oninput='onSearchInputChanged(" + order_id + ");'><div id='search_autocomplete_" + order_id + "'></div>";
    checkboxHTML += "</div>";
    if (i == 9) {
      checkboxHTML += "<button onclick='show_more_column_checkboxes(true);' id=\"show_more_column_checkboxes\" style='width: 200px;'>More</button>";
    }
  }
  else {
    if (_SPEC_DATA_INDEXES.includes(order_id)) {
      checkboxHTML_More += "<div>";
      // checkboxHTML_More += "<label class='checkBox_container' style='display: inline; font-size: 18px; padding-left: 0px;'>" + INDEXES_CONCAT[order_id] + "<br>Accepts Range (E.G. 1/2-50)<input type='checkbox' tabindex='-1' id=\"column_checkbox_" + order_id + "\" disabled><span class='checkmark' style='display: none;'></span></label><br>";
      checkboxHTML_More += "<label class='checkBox_container' style='display: inline; font-size: 18px; padding-left: 0px;'>" + INDEXES_CONCAT[order_id] + "<br>Accepts Range (E.G. 1/2-50)</label><br>";
      checkboxHTML_More += "<input id='search_input_" + order_id + "' type='text' style='width: 230px;' onfocus='deselectTable(" + order_id + ");' onfocusout='onSearchInputFocusOut();' oninput='onSearchInputChanged(" + order_id + ")'><div id='search_autocomplete_" + order_id + "'></div>";
      checkboxHTML_More += "</div>";
    }
    else {
      checkboxHTML_More += "<div>";
      // checkboxHTML_More += "<label class='checkBox_container' style='display: inline; font-size: 18px; padding-left: 0px;'>" + INDEXES_CONCAT[order_id] + "<input type='checkbox' tabindex='-1' id=\"column_checkbox_" + order_id + "\" disabled><span class='checkmark' style='display: none;'></span></label><br>";
      checkboxHTML_More += "<label class='checkBox_container' style='display: inline; font-size: 18px; padding-left: 0px;'>" + INDEXES_CONCAT[order_id] + "</label><br>";
      checkboxHTML_More += "<input id='search_input_" + order_id + "' type='text' style='width: 230px;' onfocus='deselectTable(" + order_id + ");' onfocusout='onSearchInputFocusOut();' oninput='onSearchInputChanged(" + order_id + ")'><div id='search_autocomplete_" + order_id + "'></div>";
      checkboxHTML_More += "</div>";
    }
    // if ((i + 1) % 5 == 0 && i != 0) {
    ...
    //     checkboxHTML_More += "<div><input id='search_input_" + order_id2 + "' type='text' style='width: 230px;' onfocus='deselectTable(" + order_id2 + ");' onfocusout='onSearchInputFocusOut();' oninput='onSearchInputChanged(" + order_id2 + ")'><div id='search_autocomplete_" + order_id2 + "'></div></div>";
    //   }
    //   checkboxHTML_More += "<br><br><br>";
    // }
    if (i == INDEXES_CONCAT.length - 1) {
      checkboxHTML_More += "<br><br><br><button onclick='show_more_column_checkboxes(false);' style='width: 100px; margin-left: 10px; margin-right: 10px;'>Less</button>";
      checkboxHTML_More += "<button id='search_specific_button2' style='display: inline; width: 100px; margin-top: 25px; margin-left: 10px; margin-right: 10px;' onclick='search_query();'>Go</button>";
    }
  }
}
...
var _SEARCH_ELES = ["search_input"];

for (var i = 0; i < INDEXES_CONCAT.length; ++i) {
  var ele = document.getElementById("search_input_" + i);
  if (ele != null)
    _SEARCH_ELES.push("search_input_" + i);
}
...
function openFirebaseServerScreen() {
  ...
  if (_FIREBASE_LOGGED_IN) {
    ...
    loadContentDiv1(function () {
      document.getElementById("server_select_div").style.display = "";
    });
    setKeyboardShortcutBar();
  } else {
    document.getElementById("server_select_div").style.display = "none";
...
function log_in(soft) {
  ...
  if (soft && _FIREBASE_LOGGED_IN) {
    loadContentDiv1(function () {
      ...
      // document.getElementById("local_server_connect_div").style.display = "";
    });
    setKeyboardShortcutBar();
  } else {
...
function loadContentDiv1(error_func) {

  let postSubscriptionCheck = function () {
    cancelWLMAYPDF();
    if (_FIREBASE_LOGGED_IN)
    ...
    readFromDB('open_data/admin_uid', function (val0, key0) {
      ...
      _subscribed_mode = _firebaseAuthUID != _admin_uid && !_LOCAL_SERVER_MODE;
  ...
  if (_LOCAL_SERVER_MODE) {
    postSubscriptionCheck();
  }
  else
    if (_FIREBASE_LOGGED_IN) {
      var reffFirestore = firebase.database().ref().child("firestore").child("customers").child(firebase.auth().currentUser.uid);
      reffFirestore.get().then((dataSnapshot) => {
        let firestoreCustomer = dataSnapshot.val();
        if (firestoreCustomer != null && firestoreCustomer.status != null) {
          if (firestoreCustomer.status == "trialing" && firestoreCustomer.trial_end != null && firestoreCustomer.trial_end < (new Date()).getTime()) {
            if (error_func)
              error_func();
            showSnackbar("Your free trial has expired. <a href='./stripe.html' style='color: lightblue;'>Subscribe here</a>", 10000);
          }
          else
            postSubscriptionCheck();
        }
        else {
          checkForFirestoreSubscription(firebase.auth().currentUser.uid, function () {
            if (error_func)
              error_func();
          });
        }
  ...
  var func = function () {
    ...
    if ((_subscribed_mode && !_writeable_mode) || _current_employee != null) {
      ...
      if (_subscribed_mode && !_writeable_mode) {
        ...
        var cellsAdded = 0;
        for (var i = 0; i < length; ++i)
          if (TAB_MAINMENU_DIVS[i] != "" && (_current_employee.admin || i != TAB_PEOPLE)) {
            if (cellsAdded == 0)
              shortcuts_html += "<tr>";
            shortcuts_html += '<td style="background-color: #70A2FF; border: 0px;">' + TAB_MAINMENU_DIVS[i] + "</td>";
            ++cellsAdded;
      ...
      if (_DEBUG_LOCAL_MODE) {
        ...
        document.getElementById("main_loading_text").innerHTML = "Downloading Part Child Database... 1/9";
        var bogus_dir = "";
        ...
        for (var i = 0; i < _EXTRA_DB.length; ++i) {
          readFromDB("parts_db/" + bogus_dir + _EXTRA_DB[i], function (val0, key0) {
            ...
            ++_extraDBLoadedIndex;
            document.getElementById("main_loading_text").innerHTML = "Downloading Part Child Database... " + (_extraDBLoadedIndex + 2) + "/" + _EXTRA_DB.length;
      ...
      addDBListener("trucks", function (val, key) {
        _truck_data = val;
        populateTrucksDiv();
      });
...
function send_forgot_password_email() {
  var email = document.getElementById("forgot_password_input").value;
  firebase.auth().sendPasswordResetEmail(email)
    .then(() => {
      showSnackbar("Password reset email has been sent, you may need to check your spam folder", 5000);
      document.getElementById("forgot_password_input").value = "";
      exitForgotPasswordScreen();
    })
    .catch((error) => {
      // var errorCode = error.code;
      var errorMessage = error.message;
...
function forgot_password_clicked() {
  document.getElementById("login_div").style.display = "none";
  document.getElementById("forgot_password_div").style.display = "";
}

function exitForgotPasswordScreen() {
  document.getElementById("login_div").style.display = "";
  document.getElementById("forgot_password_div").style.display = "none";
}

// var fileInput = document.getElementById('barcode_input');
// fileInput.addEventListener('change', function (e) {
//   // Create a barcode reader instance
//   const codeReader = new ZXing.BrowserBarcodeReader();

//   // Decode a QR code from an image
//   document.getElementById("barcode_img").src = URL.createObjectURL(e.target.files[0]);
//   const img = document.getElementById("barcode_img").cloneNode(true);
...
//Quagga always returns barcode not found for static images
// var fileInput = document.getElementById('barcode_input');
...
//     numOfWorkers: 0,  // disable worker to run in main thread
//     decoder: {
...
//         "i2of5_reader",
//         "2of5_reader",
//         "code_93_reader",] // type of barcode reader to use
//     },
...
//       document.getElementById("quagga_viewport").textContent = result.codeResult.code;
//     } else {
