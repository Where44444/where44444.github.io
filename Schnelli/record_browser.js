...
let page_nav_record_browser = "<button style='width: 100px; height: 40px;' onclick='pageUp();'>&lt; Prev Page</button>&nbsp;&nbsp;&nbsp;<button style='width: 100px; height: 40px;' onclick='pageDown();'>Next Page &gt;</button>";
...
function populateRecordBrowser(indexStart, highlight_IndexStart_Green, populateFromBottom) {
  ...
  if (_content.length > 0) {
    ...
    tableHTML += "</tbody></table>" + page_nav_record_browser;
...
function saveEditRecord(rownum) {
  ...
  if (!_DEBUG_LOCAL_MODE) {
    ...
    writeToDatabase('parts_db/P&A_PRI/' + row[row.length - 1], partObj, true, true, false, null, rownum);
...
function saveNewRecord() {
  ...
  if (!_DEBUG_LOCAL_MODE) {
    ...
    for (var i = 0; i < _MEMO_INDEXES.length; ++i)
      partObj[_MEMO_INDEXES[i]] = row[i + _INDEXES.length];
    writeToDatabase('parts_db/P&A_PRI/' + newPartRef.key, partObj, true, true, false, null, _content.length - 1);
    writeToChangeHistory("Add | Parent Record", "New Parent Record with OEM_PN \"" + partObj.OEM_PN + "\"");
  }
}

function copyParentRecord(key, row0) {
  var rownum = getParentIndexFromID(key);
  var partsListRef = getDatabaseRef('parts_db/P&A_PRI');
  var newPartRef = (_largestRecordNumber + 1) + "n";
  if (!_DEBUG_LOCAL_MODE)
    newPartRef = partsListRef.push();
  ...
  var row = _content[_content.length - 1];
  if (row0 == null) { //Copy Parent Record
    for (var i = 0; i < INDEXES_CONCAT.length; ++i)
      row[i] = _content[rownum][i];
  }
  else {
    if (row0 != null) {
  ...
  row[_RECORD_NUMBER] = _largestRecordNumber + 1;
  ...
  if (!_DEBUG_LOCAL_MODE) {
    ...
    for (var i = 0; i < _MEMO_INDEXES.length; ++i)
      partObj[_MEMO_INDEXES[i]] = row[i + _INDEXES.length];
    writeToDatabase('parts_db/P&A_PRI/' + newPartRef.key, partObj, true, true, false, null, _content.length - 1);
    writeToChangeHistory("Add | Parent Record", "Copy Parent Record with OEM_PN \"" + partObj.OEM_PN + "\"");
  }

}

function newParentRecord(row) {
  copyParentRecord(null, row);
}
