...
function populateSortOrders() {
  ...
  for (var i = 0; i < _sort_orders.length; ++i) {
    var display = "none";
    if (!isMobileDevice() || _sort_orders[i].showOnMobile)
      display = "";
    var quickLetter = "0";
    var keyCode = "";
    ...
    var id1 = i + 1;
    newHTML += "<tr id='sort_order_row_" + id1 + "' style='display: " + display + ";'><td>" + quickLetter + "</td><td>" +
      "<div style='display: none; align-items: center;' id='sort_order_buttons_" + id1 + "'>" +
      ...
      "<p style='font-size: 20px;'>Description&nbsp;</p><textarea id='sort_order_desc_" + id1 + "' type='text' style='width: 500px; font-size: 20px;' onfocus='deselectTable();'></textarea>" +
      "<label class='checkBox_container' style='display: block;'>Show on Mobile<input type='checkbox' id='sort_order_mobile_" + id1 + "'><span class='checkmark'></span></label>" +
      "</div>" +
      "</div>" +
    ...
    newHTML += "<button id='sort_order_sort_button_" + id1 + "' style='font-size: 20px;' onclick='sortContentBySortOrder(" + i + ");'>Sort</button>&nbsp;&nbsp;&nbsp;&nbsp;" +
      "<img class='clickable' onclick='showSortDesc(" + i + ");' width='30' height='30' src='./help.png' style='margin-bottom: -10px;' title='" + getHTMLSafeText(_sort_orders[i].desc) + "'>&nbsp;&nbsp;&nbsp;<p style='display: inline; font-size: 20px;' id='sortText_" + i + "'>" + getHTMLSafeText(_sort_orders[i].name) + "</p>" +
      "</div>" +
      "</td>";
    ...
    shortcut_window_html += "<span style='color: white;'>" + quickLetter + "</span>. ." +
      "<img width='30' height='30' src='./help.png' style='margin-bottom: -10px;' title='" + getHTMLSafeText(_sort_orders[i].desc) + "'>&nbsp;&nbsp;&nbsp;" +
      getHTMLSafeText(_sort_orders[i].name) + "<br>";
...
var _sortOrderDescSetMap = new Map();
function showSortDesc(i) {
  var ele = document.getElementById("sortText_" + i);
  if (ele != null && _sort_orders.length > i) {
    if (_sortOrderDescSetMap.has(i)) {
      ele.innerHTML = getHTMLSafeText(_sort_orders[i].name);
      _sortOrderDescSetMap.delete(i);
    }
    else {
      ele.innerHTML = getHTMLSafeText(_sort_orders[i].desc).replace(/\n/g, "<br>");
      _sortOrderDescSetMap.set(i, true);
    }
  }
}
...
function sortContentBySortOrder(order_index) {
  ...
  //   setTab(_selected_tab);
  // }
  showSnackbar("Database sorted", 3000);
}
...
function startNewSortOrder(id1) {
  var newHTML = "<table><tr id='sort_order_row_" + id1 + "'><td style='width: 600px;'>" +
    ...
    "<p style='font-size: 20px;'>Description&nbsp;</p><textarea id='sort_order_desc_0' type='text' style='width: 500px; font-size: 20px;' onfocus='deselectTable();'></textarea>" +
    "<label class='checkBox_container' style='display: block;'>Show on Mobile<input type='checkbox' id='sort_order_mobile_0' checked='true'><span class='checkmark'></span></label>" +
    "</div>" +
    "</div>" +
...
function appendSortOrderCell(id1, id2) {
  ...
  var savedDesc = document.getElementById("sort_order_desc_" + id1).value;
  for (var i = 0; i <= id2; ++i)
  ...
  document.getElementById("sort_order_desc_" + id1).value = savedDesc;
}
...
function saveNewSortOrder() {
  ...
  if (sorted_indexes.length > 0) {
    ...
    sortObj.desc = document.getElementById("sort_order_desc_0").value;
    sortObj.showOnMobile = document.getElementById("sort_order_mobile_0").checked;
    sortObj.sorted_indexes = sorted_indexes;

    var sortListRef = getDatabaseRef('sort_orders');
    var newSortOrderRef = sortListRef.push();
    writeToDatabase('sort_orders/' + newSortOrderRef.key, sortObj, false, false, false, null, null);
...
function startEditSortOrder(id1) {
  ...
  document.getElementById("sort_order_desc_" + id1).value = _sort_orders[id1 - 1].desc;
  document.getElementById("sort_order_mobile_" + id1).checked = _sort_orders[id1 - 1].showOnMobile;
  var i = 0;
...
function saveEditSortOrder(id1) {
  ...
  if (sorted_indexes.length > 0) {
    ...
    sortObj.desc = document.getElementById("sort_order_desc_" + id1).value;
    sortObj.showOnMobile = document.getElementById("sort_order_mobile_" + id1).checked;
    sortObj.sorted_indexes = sorted_indexes;

    writeToDatabase('sort_orders/' + _sort_orders[id1 - 1].key, sortObj, false, false, false, null, null);
